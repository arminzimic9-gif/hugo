...
let cachedNoiseData: Uint8Array | null = null;
...
function generateNoiseTexture(size = NOISE_SIZE) {
...
void main() {
  ...
  float eyeEnergy = clamp(max(innerRing + innerEye, outerEyeGlow + outerBgGlow) - pupil, 0.0, 3.0);
  vec3 color = uEyeColor * uIntensity * eyeEnergy;
  color += uBgColor * eyeEnergy * 0.02;
  float alphaMask = clamp(eyeEnergy * 1.25, 0.0, 1.0);
...
export default function EvilEye({
  ...
  useEffect(() => {
    ...
    if (!cachedNoiseData) {
      cachedNoiseData = generateNoiseTexture(NOISE_SIZE);
    }
    const noiseData = cachedNoiseData;
    ...
    const update = (time: number) => {
      ...
      if (!program) return;
      if (time - lastRenderAt < minFrameTime) return;
      lastRenderAt = time;
