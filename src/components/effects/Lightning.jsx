...
const Lightning = ({ hue = 230, xOffset = 0, speed = 1, intensity = 1, size = 1 }) => {
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      const nextWidth = Math.max(1, canvas.clientWidth | 0);
      const nextHeight = Math.max(1, canvas.clientHeight | 0);
    ...
    const render = (now) => {
      ...
      gl.uniform1f(uHueLocation, params.hue);
      gl.uniform1f(uXOffsetLocation, params.xOffset);
      gl.uniform1f(uSpeedLocation, params.speed);
      gl.uniform1f(uIntensityLocation, params.intensity);
      gl.uniform1f(uSizeLocation, params.size);
